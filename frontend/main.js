...
const TOKEN_CONTRACT_ADDRESS = "0x1E3CcEe7113D54523674B7aEC39D8f7805FA3DB0";
const MARKETPLACE_CONTRACT_ADDRESS = "0x7cAF98F2a88fBf5Ea4803D75564A82EEf1EA7cB2";
  ...
  userItem.getElementsByTagName("button")[0].onclick = async () => {
    ...
    await marketplaceContract.methods
      .addItemToMarket(
        item.tokenId,
        item.tokenAddress,
        userItem.getElementsByTagName("input")[0].value
      )
...
renderItem = (item) => {
  ...
  itemForSale.getElementsByTagName(
    "button"
  )[0].innerText = `Buy for ${item.askingPrice}`;
